...
class XGBRegressorCV(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, **fit_params):
        ...
        params = {
            ...
            'max_leaves': 5 * 2**(self.max_depth - 3),
...
model = GridSearchCV(
    estimator=XGBRegressorCV(n_estimators=2000, max_depth=5), 
...
joblib.dump(model, './cv/cv-5/model.pkl')

cv_res = pd.DataFrame(model.cv_results_)
cv_res.to_excel('./cv/cv-5/cv_5_results.xlsx')